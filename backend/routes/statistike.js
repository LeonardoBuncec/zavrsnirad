...
router.get('/', async (req, res) => {
  try {
    const [[ukupnoNarudzbi]] = await db.query(
      'SELECT COUNT(*) as ukupno FROM Narudzba'
    );

    const [[ukupnoProhoda]] = await db.query(
      'SELECT COALESCE(SUM(sn.kolicina * a.cijena), 0) as prihod FROM Stavka_narudzbe sn JOIN Artikl a ON sn.id_artikl = a.id_artikl'
    );
    ...
    const [najprodavanijiArtikli] = await db.query(`
      SELECT a.naziv, k.naziv AS kategorija, SUM(sn.kolicina) as prodano, SUM(sn.kolicina * a.cijena) as prihod
    ...
    const [prihodPoKategoriji] = await db.query(`
      SELECT k.naziv AS kategorija, SUM(sn.kolicina) as prodano, SUM(sn.kolicina * a.cijena) as prihod
      FROM Stavka_narudzbe sn
      JOIN Artikl a ON sn.id_artikl = a.id_artikl
      JOIN Kategorija k ON a.id_kategorija = k.id_kategorija
    ...
    const [narudzbePoSatu] = await db.query(`
      SELECT HOUR(vrijeme) as sat, COUNT(*) as narudzbi
      FROM Narudzba
      GROUP BY HOUR(vrijeme)
      ORDER BY sat
    `);
